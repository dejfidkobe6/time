...
if (isset($_GET['login'])) {
    if (!defined('GOOGLE_CLIENT_ID') || !GOOGLE_CLIENT_ID) {
        die('Google OAuth není nakonfigurován (chybí GOOGLE_CLIENT_ID v secrets.php).');
    }
...
if (!$code || !$state || $state !== ($_SESSION['oauth_state'] ?? '')) {
    header('Location: /login.php?error=' . urlencode('Neplatný OAuth stav. Zkuste to znovu.'));
    exit;
}
...
if (!$tokenRes || empty($tokenRes['access_token'])) {
    header('Location: /login.php?error=' . urlencode('Nepodařilo se získat přístupový token od Google.'));
    exit;
}
...
// 1. Try to find via oauth link table
$stmt = $pdo->prepare(
...
if (!$row) {
    // 2. Try to find by matching email (link existing account)
    ...
    if ($row) {
        ...
        try {
            ...
            $stmt->execute([$name, $email, $avatarColor]);
            $newUserId = (int)$pdo->lastInsertId();

            $stmt = $pdo->prepare(
                "INSERT INTO user_oauth (user_id, provider, provider_id) VALUES (?, 'google', ?)"
            );
            $stmt->execute([$newUserId, $googleId]);
            $pdo->commit();

            $row = ['id' => $newUserId];
        } catch (Exception $e) {
            $pdo->rollBack();
            header('Location: /login.php?error=' . urlencode('Chyba při vytváření účtu.'));
            exit;
        }
    }
}
...
// Update name from Google if it changed (optional)
$pdo->prepare("UPDATE users SET name = ? WHERE id = ? AND (name IS NULL OR name = '')")
    ->execute([$name, $row['id']]);

$safe = filter_var($returnUrl, FILTER_VALIDATE_URL) ? $returnUrl : '/';
if (parse_url($safe, PHP_URL_HOST) && parse_url($safe, PHP_URL_HOST) !== 'time.besix.cz') {
    $safe = '/';
}
